...
The PHQ9 and PHQ-like items were assessed at 4 timepoints: 0 = initial assessment, 1 = repeat assessment, 2 = imaging assessment, M = Mental Health Questionnaire online followup. The phenotypes from these sumstats were [coded as quantitative](https://biobank.ndph.ox.ac.uk/showcase/coding.cgi?id=504).
...
## UKB PHQ sample prevalences 

```{r}

ukb_phq_symptoms_prev <- read_table2('sumstats/UKB/PHQ9_boltlmm/ukb_phq9_bolt_samplesize.txt')

ukb_phq_symptoms_prev %>%
left_join(dsm_mdd_symptoms_labels %>% mutate(abbv=tolower(abbv)), by='abbv') %>%
arrange(ref) %>%
select(Symptom=h, Total=N, Cases=Nca, Controls=Nco, Prev.=prev)
...
A1 ~~ ca13*A3
c1 > 0.001
UKB_CIDI1 ~~ c1*UKB_CIDI1
c2 > 0.001
UKB_CIDI2 ~~ c2*UKB_CIDI2
c3a > 0.001
UKB_CIDI3a ~~ c3a*UKB_CIDI3a
c3b > 0.001
UKB_CIDI3b ~~ c3b*UKB_CIDI3b
c4a > 0.001
...
ukb_phq_kendler_neale_ortho.graph <- fit_graph(ukb_phq_kendler_neale_ortho.fit$results)
ukb_phq_kendler_neale_ortho.gv <- add_rank_same(generate_dot(ukb_phq_kendler_neale_ortho.graph), 1:3, 4:12)
grViz(ukb_phq_kendler_neale_ortho.gv)
...
symptoms_efa <- factanal(covmat=symptoms_cov_pd, factors=3, rotation='promax')
