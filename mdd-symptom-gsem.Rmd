...
AGDS+PGC+ALSPAC+UKB sumstats, plus UKB Touchscreen cardinal symptoms
```{r covstruc_all, results='hide'}
...
if(!file.exists(all_covstruct_r)) {
  ...
  # list sumstats distribution directories
  all_sumstats_files <- list.files(file.path('meta', 'munged'), '(ALL|UKBt).+sumstats\\.gz$', full.names=TRUE)

  # pull out which cohorts and symptom 'x' this is from the filename (COHORTS_MDDx_*)
  all_cohorts_symptoms <- str_match(basename(all_sumstats_files), '([A-Za-z_]+).(MDD[:digit:](a|b)?)')[,1]
  ...
  all_sumstats_prevs <- 
    ...
    mutate(symptom=str_match(sumstats, "[A-Za-z]\\.(MDD[1-9a-b]+)_")[,2]) %>%
    left_join(pop_prevs_w, by='symptom') %>%
    filter(!symptom == 'MDD5b') |>
    mutate(pop_prev=case_when(trait_name == 'UKBt.MDD1' ~ 0.557,
                              trait_name == 'UKBt.MDD2' ~ 0.368,
                              TRUE ~ pop_prev))
...
Full meta analysis only

```{r mdd_symptom_gsem_h2}

ggplot(filter(sumstats_h2_labels, cohorts=='All'),
        aes(x=abbv,
            y=h2,
            ymin=h2+se*qnorm(0.025),
            ymax=h2+se*qnorm(0.975))) +
...
ggsave('mdd-symptom-gsem_files/symptoms_h2_snp_all.png', width=8, height=4)
