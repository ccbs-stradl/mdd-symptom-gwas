...
daner_files <- list.files("meta/distribution", pattern="^daner_[A-Z_]+\\.MDD[1-9a-b]+_[a-z]+\\.gz$", full.name=TRUE, recursive=TRUE)
...
covstruct_prefix <- 'all.covstruct'
covstruct_r <- file.path('ldsc', paste(covstruct_prefix, 'deparse.R', sep='.'))
covstruct_rds <- file.path('ldsc', paste(covstruct_prefix, 'rds', sep='.'))
...
if(!file.exists(covstruct_r)) {
  ...
  # list sumstats distribution directories
  sumstats_files <- list.files(file.path('meta', 'munged'), 'ALL.+sumstats\\.gz', full.names=TRUE)
