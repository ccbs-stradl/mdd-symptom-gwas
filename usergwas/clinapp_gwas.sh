# reformat Clin Dec/AppInc symptoms gwas into commongwas columns for 
...
zcat meta/distribution/AGDS_PGC.MDD3a_weightLoss/daner_AGDS_PGC.MDD3a_weightLoss.gz| awk '{if(NR == 1) {print "SNP", "CHR", "BP", "MAF", "A1", "A2", "est", "se_c", "Pval_Estimate"} else {print $2, $1, $3, $6, $4, $5, log($9), $10, $11}}' | gzip -c > meta/usergwas/agds_pgc.appdec.sumstats.txt.gz 