...
cat("\n")
  if(j == d){
	...
	gwas.df <- gwas.df %>% filter(SNP %in% snps.name.list) %>% filter(!is.na(N))
  ...
  gwas1.df <- gwas1.df %>% filter(SNP %in% snps.name.list) %>% filter(!is.na(N))
  gwas2.df <- gwas2.df %>% filter(SNP %in% snps.name.list) %>% filter(!is.na(N))
