...
phenotypes |> mutate(across(MDD1:MDD9, ~ case_when(.x == 1 ~ 2L,
                                             .x == 0 ~ 1L,
                                             TRUE ~ -9L))) |>
select(FID=ID1, IID=ID2, MDD1:MDD9)

write_tsv(pheno, file.path(output_dir, "mddw2_symptoms.pheno"))
```
...
cases <- pheno |>
filter(str_detect(FID, "^cas_mdd")) |>
mutate(dataset = sapply(str_split(FID, pattern = "_", n = 4), function(id) id[3]))
...
cohorts=$(cat pgc_gwas/cases.id | awk '{print $1}' | awk -F_ '{print $3}' | sort | uniq)
echo $cohorts
...
for cohort in $cohorts; do
  if [ -f $V1/cobg_dir_genome_wide/mdd_${cohort}_eur_sr-qc*.hg19.ch.fl.bgn.bed ]; then
    echo Linked $cohort
    ln -sf $V1/cobg_dir_genome_wide/mdd_${cohort}_*.bgn.* pgc_gwas/cobg_dir_genome_wide/
  elif [ -f $V2/cobg_dir_genome_wide/mdd_${cohort}_eur_sr-qc2.hg19.ch.fl.bgn.bed ]; then
    echo Linked $cohort
    ln -sf $V2/cobg_dir_genome_wide/mdd_${cohort}_*.bgn.* pgc_gwas/cobg_dir_genome_wide/
  else
    echo Did not link $cohort
  fi
done

# patch in cof3 (cof2) and rau2 (raus)
