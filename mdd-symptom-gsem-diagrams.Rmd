...
```{r coefs}
model_coefs <- read_tsv("mdd-symptom-gsem_files/model_coefs.txt") |>
...
model_coefs_symptom_residuals <- model_coefs_symptoms |>
  mutate(lhs = str_glue("u[{lhs}]"), op = "=~",
         STD_Genotype = 1, STD_Genotype_SE = 0)
...
for(model_letter in names(model_coefs_list)) {
  ...
  cat(str_replace_all(model.gv, "'", '"'),
      file=str_glue('mdd-symptom-gsem-diagrams_files/{model_letter}.gv'))
}

```

```{sh dot_images}
for DOT in mdd-symptom-gsem-diagrams_files/*.gv; do
