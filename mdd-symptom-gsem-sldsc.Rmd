...
dir.create(covstruct_dir)

for(entry in c('S', 'V', 'S_Tau', 'V_Tau')) {

	cat(paste(entry, '\n'))
	entry_dir <- here::here(covstruct_dir, entry)
	dir.create(entry_dir)
		
	for(annot in names(symptoms_strat_covstruct[[entry]])) {
		annotation_matrix <- symptoms_strat_covstruct[[entry]][[annot]]
		annotation_matrix_r <- here::here(entry_dir, paste(annot, 'deparse', 'R', sep='.'))
		dput(annotation_matrix, annotation_matrix_r, control=c('all', 'digits17'))	
	}

}


dput(symptoms_strat_covstruct[c('I', 'N', 'm', 'Prop', 'Select')], covstruct_r, control=c('all', 'digits17'))
